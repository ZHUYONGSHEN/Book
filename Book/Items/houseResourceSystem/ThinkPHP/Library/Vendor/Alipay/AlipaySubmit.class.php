...
class AlipaySubmit {
	...
	function __construct($_config = array()){
		...
		require_once($this->curpath."/alipay.config.php");
		require_once($this->curpath."/lib/alipay_core.function.php");
		require_once($this->curpath."/lib/alipay_rsa.function.php");
		require_once($this->curpath."/lib/alipay_md5.function.php");
		
		$this->alipay_config = array_merge($alipay_config,$_config);
	}


    function makeAppPayStr($data){
    		$para_filter['partner'] = '"'.$this->alipay_config['partner'].'"';
			$para_filter['seller_id'] = '"'.$this->alipay_config['seller'].'"';
			$para_filter['out_trade_no'] = '"'.$data['order_no'].'"';
			$para_filter['subject'] = '"'.$data['subject'].'"';
			$para_filter['total_fee'] = '"'.sprintf("%.2f",$data['total_fee']).'"';//金额
			$para_filter['body'] = '"'.$data['body'].'"';//商品描述
			$para_filter['notify_url'] = '"'.$this->alipay_config['callback_url'].'"';
			$para_filter['service'] = '"mobile.securitypay.pay"';
			$para_filter['payment_type'] = '"1"';
			$para_filter['_input_charset'] = '"utf-8"';
			$para_filter['it_b_pay'] = '"30m"';
			...
			$sign = urlencode($sign);
			return $prestr.'&sign="'.$sign.'"&sign_type="'.$sign_type.'"';
    }
